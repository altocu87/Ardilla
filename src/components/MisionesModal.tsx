      ...
      >
        ...
        <div className="px-5 pt-2 pb-3 shrink-0">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex flex-col items-end`}>
              <span className={`text-2xl font-black ${allDone ? "text-amber-500" : "text-slate-300"}`}>
                {allDone ? "🏆" : `${completedCount}/3`}
              </span>
              {allDone && (
                <span className="text-[10px] font-bold text-amber-600 bg-amber-100 px-2 py-0.5 rounded-full">
                  +20🌰 bonus desbloqueado
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mt-3 h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-700 ${
                allDone ? "bg-gradient-to-r from-amber-400 to-yellow-500" : "bg-gradient-to-r from-teal-400 to-emerald-500"
              }`}
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4 flex flex-col gap-3">
          ...
          <div className={`rounded-2xl border-2 px-4 py-3 flex items-center gap-3 transition-all ${
            ...
            <div>
              <p className={`font-bold text-sm ${allDone ? "text-amber-800" : "text-slate-500"}`}>
                Bonus por completar las 3
              </p>
              <p className={`text-xs ${allDone ? "text-amber-600" : "text-slate-400"}`}>
                +20🌰 y +30 XP extra cuando acabes todas
              </p>
            </div>
          </div>
        </div>
